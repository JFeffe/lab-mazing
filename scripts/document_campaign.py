...
titles={1:'Le laboratoire',2:'Le département des machines',3:'Le département d’optique',4:'Le département des essais',5:'Le défi de Folamour',6:'Les serres expérimentales',7:'Le service des photocopies'}
...
def make_map(n,m,E,S):
 ...
 txt(65,30,f'FOLAMOUR / V{VERSION} / '+('CHAPITRE 2 / MISSION '+str(n-5) if n>=6 else f'NIVEAU {n}')+' / SPOILERS',27,'#e9be72')
 txt(65,80,titles[n].upper(),44)
 txt(65,143,'Raccourcis révisés : revenir plus vite dans les couloirs déjà parcourus',24)
 ox,oy,c=95,220,38
 palettes={1:['#648a83','#7b83a4','#9e8174'],2:['#9e8460','#527f8e','#858371'],3:['#7789a5','#638f94','#93839e'],4:['#a18d72','#788ba2','#6b968d'],5:['#659b9d','#ac9070','#8982a9'],6:['#729d77','#82aaa0','#af986d'],7:['#9aa7b0','#b19c7b','#839c9a']}
 for y,row in enumerate(m['grid']):
  for x,v in enumerate(row):
   zone=(2 if y>=20 else 0 if x<17 else 1) if n==7 else (0 if x<13 else 1 if x>23 else 2) if n==6 else (2 if y<12 else 0 if x<15 else 1 if x>19 else 2) if n==5 else (0 if y<(11 if n==1 else 12) else 1 if y<24 else 2)
