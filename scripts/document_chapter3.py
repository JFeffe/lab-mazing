"""Five source-exact maps and French solution guides for chapter three."""
...
font=str(R/'game/assets/Interface.ttf');pdfmetrics.registerFont(TTFont('Interface',font));design=json.loads((R/'scripts/chapter3_design.json').read_text());guidance=json.loads((D/'guidance.json').read_text())
...
def zone(n,x,y):
 if n==11:return min(2,x//12)
 if n==12:return min(2,y//12)
 if n==13:return 1 if x>17 else 0 if y<17 else 2
 if n==14:return 2 if y>19 else 0 if x<17 else 1
 return 1 if x<9 else 2 if x>25 else 0
palettes=[['#ab8e68','#809a96','#8797b2'],['#a2a084','#8e9cae','#8cab98'],['#6c97a2','#8f85a4','#88a798'],['#849fa2','#b49a6d','#8e8eae'],['#9582a8','#8c9c98','#ae986f']]
for n in range(11,16):
 num=n-10;entry=design[str(n)];folder=out/f'Niveau-{num}';folder.mkdir(exist_ok=True);prefix=f'Chapitre-3-Niveau-{num}-v0.17'
 ...
 txt(65,30,f'FOLAMOUR / V0.17 / CHAPITRE 3 / NIVEAU {num} / SPOILERS',27,'#e9be72');txt(65,85,entry['title'].upper(),44,width=1890)
 ...
 for i,e in enumerate(sorted(E,key=lambda e:int(e['ref']))):txt(sx,270+i*40,e['ref']+'  '+e['title'],20,cs[e['kind']],width=490)
 ...
 for i,(t,col) in enumerate([('Or : objet à ramasser',cs['pickup']),('Turquoise : note / preuve',cs['clue']),('Bleu : mécanisme',cs['mechanism']),('Corail : sas verrouillé',cs['door']),('Blanc : sortie ; D : départ','#ffffff'),('Vert : raccourci à révéler','#54cda4'),('501, 502, 503 : notes secrètes','#ffffff')]):txt(sx,1330+i*32,t,21,col,width=490)
 ...
 txt(65,1820,'3 SECRETS / 4 RACCOURCIS / 3 ÉNIGMES / ESSAIS RÉVERSIBLES / AUCUN COMPTE À REBOURS',24,'#e9be72')
 ...
 story=[];md=[f'# {entry["title"]}\n\nChapitre 3 / Niveau {num} / Version 0.17 / SPOILERS\n']
 def p(t,style='body'):
 ...
 page(entry['title']+f'\nChapitre 3 / Niveau {num}')
 p('SPOILERS / VERSION 0.17','head');p(entry['intro']);p(entry['layout'])
 p('Parcours conseillé','head')
 p('Menu → Choisir un chapitre → Chapitre 3, puis choisir ce niveau. La transition depuis la mission précédente conserve les bilans. Le sac et le journal repartent à zéro à chaque nouveau niveau.')
 ...
 p('Les notes secrètes 501, 502 et 503 sont facultatives. Leur lecture compte au bilan. Les quatre raccourcis demandent de marcher des deux côtés du mur ; la simple visibilité sur la carte ne suffit pas.')
 ...
 p('Raccourcis facultatifs','head');table([['ID','Mur','Deux côtés à visiter','Gain minimal*']]+[[s['id'],str(tuple(s['cell'])),' ↔ '.join(str(tuple(c)) for c in s['sides']),str(s['minimum_saved_steps'])+' pas'] for s in S],[45,85,277,100]);p('* Gain sur le trajet entre les deux côtés, même avec les autres raccourcis ouverts. Le parcours principal fonctionne sans ouvrir aucun raccourci.')
 ...
 p('La suite','head');p('Le bouton « Continuer la mission suivante » enchaîne le niveau suivant et conserve le bilan.' if n<15 else 'Fin du chapitre 3. Le chapitre 4 est annoncé, mais n’est pas encore jouable dans la version 0.17.')
 p('Vérification du parcours','head');p(f'Le trajet de référence parcourt {sum(len(s["route"])-1 for s in route)} cases de déplacement et visite les quinze points interactifs, dont les trois secrets. Les deux sas portent le total à dix-sept repères. Les solutions du guide sont utilisées par les tests des commandes réelles du jeu.')
 def footer(c,d):
  c.setFont('Interface',8);c.setFillColor(colors.HexColor('#647e70'));c.drawString(44,25,f'FOLAMOUR / V0.17 / CHAPITRE 3 - NIVEAU {num} / SPOILERS');c.drawRightString(A4[0]-44,25,str(d.page))
...
summary=['# Lab-Mazing v0.17 — Le département de la prévoyance\n','Cinq niveaux jouables, quinze nouveaux essais, quinze notes secrètes et vingt raccourcis. Français / anglais.\n','[Jouer](https://jfeffe.github.io/lab-mazing/)\n','## Les cinq missions\n']
for n,d in design.items():summary.append(f'### {int(n)-10}. {d["title"]}\n\n{d["layout"]}\n\n'+ '\n'.join('- '+s['solution'] for s in d['sections'])+'\n')
summary+=['## Progression narrative\n','MIROIR et les copies de Folamour alimentent une bureaucratie qui prévoit, surveille et finit par fabriquer ses propres crises. Le conseil interprète l’absence de menace identifiable comme la preuve de sa discrétion. La dernière scène annonce une cité souterraine : le complexe de la certitude, futur chapitre 4.\n','## Contenu\n','Chaque dossier de niveau contient la carte exacte en PNG et PDF A3, ainsi que le guide détaillé en PDF A4 et Markdown. Les documents contiennent les solutions. Les versions précédentes restent archivées.\n']
(out/'Resume-v0.17.md').write_text('\n'.join(summary))
with zipfile.ZipFile(out/'Lab-Mazing-v0.17-Chapitre-3-Cheatsheets.zip','w',zipfile.ZIP_DEFLATED) as z:
