extends RefCounted
const Finale=preload("res://FinaleControls.gd")
const Certainty=preload("res://CertaintyControls.gd")
const Foresight=preload("res://ForesightControls.gd")
const Archives=preload("res://ArchiveControls.gd")
...
static func initial(e):
	if e.puzzle_type=="finale":return Finale.initial(e)
	if e.puzzle_type=="certainty":return Certainty.initial(e)
	if e.puzzle_type=="foresight":return Foresight.initial(e)
	if e.puzzle_type in Archives.TYPES:return Archives.initial(e)
...
static func solved(g,e):
	...
	if e.puzzle_type=="finale":return Finale.solved(g,e,values)
	if e.puzzle_type=="certainty":return Certainty.solved(g,e,values)
	if e.puzzle_type=="foresight":return Foresight.solved(e,values)
	if e.puzzle_type in Archives.TYPES:return Archives.solved(e,values)
...
static func render(g,e):
	var values=state(g,e)
	if e.puzzle_type=="finale":Finale.render(g,e,values)
	if e.puzzle_type=="certainty":Certainty.render(g,e,values)
	if e.puzzle_type=="foresight":Foresight.render(g,e,values)
	if e.puzzle_type in Archives.TYPES:Archives.render(g,e,values)
...
static func move(g,e,index):
	...
	if e.puzzle_type=="finale" and not Finale.can_move(g,e):
		g.toast("Modifier ces accès exige de revenir au pupitre.")
		return
	if e.puzzle_type=="finale" and e.has("world_gates"):g.stop_navigation()
	if e.puzzle_type=="certainty" and not Certainty.can_move(g,e):
		g.toast("Modifier ces accès exige de revenir au pupitre.")
		return
	if e.puzzle_type=="certainty" and e.has("world_gates"):g.stop_navigation()
	if e.puzzle_type in Archives.TYPES and not Archives.can_move(g,e):
		g.toast("Les rayonnages ne bougent pas lorsqu’une personne se trouve entre les rangées. Revenez à la console au sud.")
		return
	if e.puzzle_type=="stacks":g.stop_navigation()
	var values=state(g,e)
	if index==-2:g.puzzle_states[e.id]=initial(e)
	elif e.puzzle_type=="finale":Finale.move(e,values,index)
	elif e.puzzle_type=="certainty":Certainty.move(e,values,index)
	elif e.puzzle_type=="foresight":Foresight.move(e,values,index)
	elif e.puzzle_type in Archives.TYPES:Archives.move(e,values,index)
