...
func action(index):
	for b in buttons(game.modal_box):
		if b.get_meta("certainty_action",-500)==index and not b.disabled:
			b.pressed.emit();await process_frame;return
...
func run():
	...
	game.start_game(false,15);game.elapsed=123;game.show_win()
	await press("Entrer dans le complexe de la certitude")
	var solutions=JSON.parse_string(FileAccess.get_file_as_string("res://tests/chapter4_solutions.json"))
	for number in range(16,21):
		...
		for step in JSON.parse_string(FileAccess.get_file_as_string("res://tests/route_level%d.json"%number)):
			...
			if e.kind in ["clue","pickup"]:game.interact(e)
				...
				game.show_puzzle(e)
				if e.has("requires") and not game.done.has("installed_"+e.id):game.install_items(e)
				if step.has("configure"):
					for i in step.configure:await action(int(i))
					game.close_modal();continue
				if e.has("puzzle_type"):
					await press("Réinitialiser cet essai")
					...
					if e.mode in controls.Certainty.PROGRAMS:
						...
						await action(99);check(controls.state(game,e).slice(0,3)==[12,0,13],"Undo seal crossing")
						await action(0);await action(1);await action(98);check(controls.solved(game,e),"Redo seal crossing")
			...
			print("CHAPTER4 ROUTE ",e.id," PASS")
		...
		check(game.won and game.level==number and game.level_stats.has("15"),"Finished resume")
		if number<20:await press("Continuer la mission suivante")
	...
	print("CHAPTER4 PHYSICS + 15 SOLUTIONS + SAVES + HINTS + TRANSITIONS: ","FAIL" if failed else "PASS")
