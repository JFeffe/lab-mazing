...
func run():
	...
	for child in game.modal_box.get_children():
		if child is Button:check(child.text!=game.loc(event("g_coffee").action),"Exit available early")
	game.close_modal()
	...
	for step in route:
		...
		if e.kind in ["clue","pickup"]:
			...
			if e.has("puzzle_type"):
				...
				for child in game.modal_box.get_children():
					if child is Button and child.text==game.loc(event("g_coffee").action):child.pressed.emit();await process_frame;break
