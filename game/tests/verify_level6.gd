...
func run():
		...
		for sign_v in [-1,1]:
			for lane in [-0.9,0,0.9]:
				...
				for frame in range(60):
					await physics_frame
					game.player.velocity=Vector3(sign_v*7,-2,0)
	...
	check("Coffee is served" in all_text and "photocopies" in all_text and "not available yet" in all_text,"First mission ending and coming next")
	game.show_title()
