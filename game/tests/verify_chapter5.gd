...
func action(index):
	for b in buttons(game.modal_box):
		if b.get_meta("finale_action",-500)==index and not b.disabled:
			b.pressed.emit();await process_frame;return
...
func run():
	...
	game.start_game(false,20);game.elapsed=123;game.show_win()
	await press("Entrer dans le programme HORIZON")
	...
	for number in range(21,26):
		...
		for step in JSON.parse_string(FileAccess.get_file_as_string("res://tests/route_level%d.json"%number)):
			...
			if e.kind in ["clue","pickup"]:game.interact(e)
				...
				if e.has("puzzle_type"):
					...
					if e.mode in controls.Finale.PROGRAMS:
		...
		check(game.won and game.level==number and game.level_stats.has("20"),"Finished resume")
		if number<25:await press("Continuer la mission suivante")
	check(game.level_stats.size()==6,"Campaign totals lost")
	await press("Regarder la surface")
	check(game.machine_parts.surface.visible and not game.modal_open,"Exterior ending missing")
	game.interact_button.pressed.emit();await process_frame
	check(game.modal_open,"Cannot reopen ending")
	game.show_chapters()
	...
	print("CHAPTER5 PHYSICS + 15 SOLUTIONS + SAVES + HINTS + TRANSITIONS: ","FAIL" if failed else "PASS")
